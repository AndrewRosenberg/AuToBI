...
/**
 * NormalizedTimeValuePairFeatureExtractor normalizes a contour represented by a list of time value pairs and aligns the
 ...
 *
 * @see edu.cuny.qc.speech.AuToBI.SpeakerNormalizationParameter
 */
public class NormalizedTimeValuePairFeatureExtractor extends FeatureExtractor {
  ...
  private List<TimeValuePair> zScoreNormalizeValues(List<TimeValuePair> values,
    ...
    for (TimeValuePair tvp : values) {
      normalized_values.add(new TimeValuePair(tvp.getTime(), norm_params.normalize(feature_name, tvp.getValue())));
    }
