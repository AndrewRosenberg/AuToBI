...
public class PitchExtractor extends SampledDataAnalyzer {
   ...
   */
  public List<TimeValuePair> soundToPitch() throws AuToBIException {
   ...
   * @throws AuToBIException if there are problems
   */
  public List<TimeValuePair> soundToPitch(double time_step, double min_pitch, double max_pitch)
   ...
   */
  public List<TimeValuePair> soundToPitchAc(double time_step, double min_pitch, double periods_per_window,
    ...
    ArrayList<TimeValuePair> pitch = new ArrayList<TimeValuePair>();
   ...
   */
  private ArrayList<TimeValuePair> pathFinder(ArrayList<PitchFrame> pitchFrames, double silenceThresh,
    ...
    for (int i = 0; i < pitchFrames.size(); ++i) {
      if (pitchFrames.get(i).getCandidate(0).getFrequency() > 0) { // voiced
        pitch.add(new TimeValuePair(indexToX(t0, time_step, i), pitchFrames.get(i).getCandidate(0).getFrequency()));
      }
    }
    return pitch;
  }
  ...
  public static void main(String[] args) throws IOException, UnsupportedAudioFileException, AuToBIException {
    ...
    for (int i = 0; i < pitch.size(); ++i) {
      System.out.println("pitch point[" + i + "]: " + pitch.get(i).getValue() + " -- " + pitch.get(i).getTime());
    }
  }
}
